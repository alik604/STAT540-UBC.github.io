...
**If you cannot download the expression data from the code below, the "eQTL_melanoma.RData" file can be downloaded [here](https://www.dropbox.com/s/0mr5s8ilo1rtsq4/eQTL_melanoma.Rdata?dl=0). You will want to add this file to your .gitignore, as it is quite large.** 
...
> NOTE: You can also use the 'rlog' transformation using the rlog function in DESeq2, which can be used for differential expression analysis. How might this transformation be better than the log transformation (what factors does it account for that log2 transform might not?).  
